...
function resolutionHandler (data)  {
  ...
  // Our handler just prints to the console and this is what we'll test
	if (err.length) console.warn(err)
	if (res.length) console.log(res);

  // We'll process the next batch if there are still ids pending
	if (data.ids.length > 0) run(data.ids);
};
...
const processAll = Future.forkCatch(console.error)(rejectionHandler)(resolutionHandler);
...
const run = (ids) => {
  ...
  // We pack all the futures into an array so we can execute them in parallel
	...
	processAll(Future.go(function* () {
		let response = yield Future.parallel(BATCH_SIZE)(promises);
